...
exports.populerRecipe = async (req, res) => {
    try {
        const number = req.query.number || 2
        RecipesMeta.find().then(allRecipe => {
            recipeArray = allRecipe
            const result = recipeArray.filter(ele => ele.visitedNumberOfTime >= number)
